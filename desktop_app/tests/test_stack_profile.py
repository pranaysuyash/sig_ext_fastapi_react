...
def test_default_profile_prefers_pikepdf_without_fitz(monkeypatch):
    ...
    _profile_with_mocks(monkeypatch, {"pypdfium2", "pikepdf", "cv2", "pytesseract"})
...
def test_opt_in_profile_allows_fitz_before_pikepdf(monkeypatch):
    ...
    _profile_with_mocks(monkeypatch, {"pypdfium2", "fitz", "pikepdf", "cv2", "pytesseract"})
...
def test_scan_preprocess_hint_profile_and_install_message(monkeypatch):
    ...
    assert stack_profile.stack_install_hint() == "pip install pytesseract"
