# Terms of Service - Signature Extractor

**Last Updated:** November 7, 2025  
**Effective Date:** November 7, 2025

## Agreement

By using Signature Extractor, you agree to these terms. If you don't agree, don't use the software.

## The Software

### What It Is

**Signature Extractor is:**
- Desktop application for signature extraction
- Local processing software (runs on your computer)
- Licensed software (not open source)
- Distributed as-is

### What It's Not

**Signature Extractor is NOT:**
- A cloud service or SaaS
- A subscription product
- A guaranteed service
- Legal advice or compliance tool

## License Grant

### What You Get

**Upon purchase, you receive:**
- Non-exclusive license to use the software
- Right to install on your personal computers
- Right to use for personal or commercial purposes
- Access to updates for your major version

### What You Don't Get

**You do NOT receive:**
- Source code or right to modify
- Right to redistribute or resell
- Right to share your license
- Ownership of the software
- Multi-user or enterprise rights

## Restrictions

### You May NOT

**Prohibited actions:**
- ❌ Share your license key with others
- ❌ Use one license for multiple users
- ❌ Reverse engineer or decompile
- ❌ Remove or bypass license restrictions
- ❌ Redistribute or resell the software
- ❌ Create derivative works
- ❌ Use for illegal purposes

### You MAY

**Allowed actions:**
- ✅ Install on multiple computers you own
- ✅ Use for personal or business purposes
- ✅ Process any documents you have rights to
- ✅ Create backups for personal use
- ✅ Use offline without internet

## Trial Mode

### Evaluation Period

**Trial mode allows:**
- Unlimited time to evaluate
- Access to all features
- Full functionality testing
- No credit card required

**Trial restrictions:**
- Export feature locked
- PDF save feature locked
- Restriction dialogs shown

**No obligation:**
- Try as long as you want
- No automatic charges
- No account required
- Delete anytime

## Payment and Refunds

### Purchase

**When you buy:**
- One-time payment of $29 USD
- Processed by Gumroad
- License key delivered via email
- Instant activation

### No Refunds

**All sales are final:**
- Try before you buy (full trial available)
- No refunds after purchase
- No exchanges or transfers
- See docs/PURCHASE_POLICY.md for details

**Exceptions:**
- Technical issues we can't fix
- Fraudulent purchases
- At our sole discretion

## Privacy

### Data Collection

**We don't collect your data:**
- All processing is local
- No cloud uploads
- No tracking or analytics
- No personal information collected

**See full policy:**
- docs/PRIVACY_POLICY.md
- https://signatureextractor.app/privacy

## Warranties and Disclaimers

### No Warranty

**THE SOFTWARE IS PROVIDED "AS IS":**
- No guarantee it will work for your needs
- No guarantee of uptime or availability
- No guarantee of accuracy or results
- No guarantee of compatibility

### Disclaimer

**WE DISCLAIM ALL WARRANTIES:**
- Express or implied warranties
- Merchantability
- Fitness for particular purpose
- Non-infringement
- Any warranties from course of dealing

**IN PLAIN ENGLISH:**
- The software might have bugs
- It might not work on your system
- Results might not be perfect
- Use at your own risk

## Limitation of Liability

### Maximum Liability

**OUR LIABILITY IS LIMITED TO:**
- Amount you paid for the license ($29)
- No consequential damages
- No indirect damages
- No lost profits or data

### What This Means

**WE'RE NOT LIABLE FOR:**
- Lost documents or data
- Business interruption
- Missed deadlines
- Legal issues from using the software
- Any damages beyond the purchase price

**IN PLAIN ENGLISH:**
- If something goes wrong, worst case we refund $29
- We're not responsible for your business losses
- You're responsible for backing up your data
- Use the software responsibly

## Indemnification

### You Agree To

**Indemnify us against:**
- Claims from your use of the software
- Your violation of these terms
- Your violation of others' rights
- Your illegal use of the software

**IN PLAIN ENGLISH:**
- If you do something wrong with the software
- And someone sues us because of it
- You'll cover our legal costs

## Acceptable Use

### Permitted Uses

**You may use the software for:**
- Personal document processing
- Business document processing
- Legal document preparation
- Any lawful purpose

### Prohibited Uses

**You may NOT use the software for:**
- Illegal activities
- Fraud or forgery
- Violating others' rights
- Creating fake documents
- Any unlawful purpose

**We're not responsible:**
- For how you use the software
- For documents you create
- For legal compliance
- For your business practices

## Intellectual Property

### Our Rights

**We own:**
- The software and all code
- The Signature Extractor name and brand
- All documentation and materials
- All intellectual property rights

### Your Rights

**You own:**
- Documents you process
- Signatures you extract
- Files you create
- Your data (stored locally)

### Third-Party Rights

**The software uses:**
- Open source libraries (see ATTRIBUTIONS.md)
- Licensed under their respective terms
- We comply with all licenses
- You must comply too

## Updates and Support

### Updates

**We may provide:**
- Bug fixes and security patches
- Minor feature improvements
- Compatibility updates
- Major version upgrades (may cost extra)

**We're not obligated to:**
- Provide any updates
- Support old versions
- Add new features
- Fix all bugs

### Support

**We provide:**
- Email support
- Bug fix assistance
- License activation help
- Documentation

**We don't provide:**
- Phone support
- Immediate support
- Custom development
- Training or consulting

## Termination

### We May Terminate

**Your license if you:**
- Violate these terms
- Share your license key
- Use for illegal purposes
- Abuse the software

**Upon termination:**
- License is revoked
- No refund provided
- Must stop using software
- Must delete all copies

### You May Terminate

**You can stop using anytime:**
- Delete the software
- No refund (all sales final)
- License remains valid (can't transfer)

## Changes to Terms

### Updates

**We may update these terms:**
- Changes posted on website
- Effective immediately
- Continued use = acceptance
- Major changes announced

**Your options:**
- Accept new terms
- Stop using software
- No refund for disagreement

## Governing Law

### Jurisdiction

**These terms governed by:**
- Laws of [Your Jurisdiction]
- Without conflict of law provisions
- Exclusive jurisdiction in [Your Courts]

### Disputes

**If there's a dispute:**
1. Contact us first: support@signatureextractor.app
2. Try to resolve amicably
3. Arbitration if needed
4. Small claims court as last resort

## Miscellaneous

### Entire Agreement

**These terms are:**
- The complete agreement
- Supersede prior agreements
- Can only be modified in writing
- Apply to all users

### Severability

**If any provision is invalid:**
- Rest of terms remain valid
- Invalid provision modified to be valid
- Intent of terms preserved

### No Waiver

**If we don't enforce a term:**
- Doesn't mean we waive it
- Can still enforce later
- Doesn't set precedent

### Assignment

**You can't transfer:**
- Your license
- Your rights under these terms
- Your obligations

**We can transfer:**
- Our rights and obligations
- To successor or acquirer
- With notice to you

## Contact

### Questions?

**Contact us:**
- Email: support@signatureextractor.app
- Website: https://signatureextractor.app
- Response: Within 24-48 hours

### Legal Notices

**Send to:**
- Email: legal@signatureextractor.app
- Must include: Your name, contact info, issue description

## Summary

**In Plain English:**

✅ **Try before you buy** - Full trial available  
✅ **One license per user** - Don't share your key  
✅ **Use responsibly** - Don't do illegal stuff  
✅ **No refunds** - Because you can try first  
✅ **Limited liability** - Max $29 if something goes wrong  
✅ **We own the software** - You get a license to use it  
✅ **Privacy-first** - All processing is local  
✅ **Fair terms** - We're reasonable people  

**Questions?** Email support@signatureextractor.app

---

**By using Signature Extractor, you agree to these terms.**

**Last Updated:** November 7, 2025
