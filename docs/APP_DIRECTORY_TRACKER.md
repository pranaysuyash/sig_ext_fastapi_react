# App Directory Submission Tracker

**Last Updated:** November 26, 2025  
**Purpose:** Track all app directory submissions for SignKit  
**Goal:** Maximum distribution and discoverability

---

## 📊 SUBMISSION STATUS OVERVIEW

| Directory | Status | Priority | Submitted | Approved | Notes |
|-----------|--------|----------|-----------|----------|-------|
| BetaList | 🟢 Submitted | 🔥🔥 High | Nov 21 | Pending | In review |
| Product Hunt | 🔴 Not Started | 🔥🔥🔥 Critical | - | - | Launch Nov 28 |
| AlternativeTo | 🟢 Submitted | 🔥🔥🔥 Critical | Nov 26 | Pending | 24-48hr review |
| Slant | 🔴 Not Started | 🔥🔥 High | - | - | Submit today |
| Capterra | 🔴 Not Started | 🔥🔥 High | - | - | 1-2 week review |
| G2 | 🔴 Not Started | 🔥🔥 High | - | - | 1-2 week review |
| MacUpdate | 🔴 Not Started | 🔥 Medium | - | - | Mac-specific |
| Softpedia | 🔴 Not Started | 🔥 Medium | - | - | All platforms |
| SourceForge | 🔴 Not Started | 🔥 Low | - | - | Optional |
| FileHorse | 🔴 Not Started | 🔥 Low | - | - | Optional |
| AppSumo | 🔴 Not Applied | 🔥🔥🔥 High | - | - | Apply Dec |

**Legend:**
- 🔴 Not Started
- 🟡 In Progress
- 🟢 Submitted
- ✅ Approved/Live
- ❌ Rejected

---

## 🎯 TIER 1: CRITICAL (Submit Today - Nov 21)

### 0. BetaList
**URL:** https://betalist.com/  
**Status:** 🟢 Submitted (Nov 21, 2025)  
**Impact:** 🔥🔥 HIGH  
**Audience:** Early adopters, startup enthusiasts  
**Cost:** Free  
**Review Time:** Varies (typically 1-3 days)

**Why This Matters:**
- Early adopter audience
- Good for initial traction
- Email list building
- Community feedback

**Submission Checklist:**
- [x] Create BetaList account
- [x] Submit product listing
- [x] Add description and screenshots
- [x] Submit for review
- [ ] Awaiting approval

**Submission Date:** November 21, 2025  
**Expected Approval:** November 22-24, 2025  
**Status:** In review

**Post-Approval Actions:**
- [ ] Monitor traffic and signups
- [ ] Engage with community feedback
- [ ] Track conversion metrics

---

### 1. Product Hunt
**URL:** https://www.producthunt.com/  
**Status:** 🔴 Not Started  
**Impact:** 🔥🔥🔥 CRITICAL  
**Audience:** 5M+ tech enthusiasts  
**Cost:** Free  
**Review Time:** Instant (scheduled launch)

**Submission Checklist:**
- [ ] Create Product Hunt account
- [ ] Verify email
- [ ] Complete maker profile
- [ ] Prepare product listing:
  - [ ] Product name: SignKit
  - [ ] Tagline: "Extract signatures from documents with complete privacy"
  - [ ] Description (see BFCM_READINESS_2025.md)
  - [ ] Upload 5-7 screenshots
  - [ ] Upload demo video (60-90 seconds)
  - [ ] Add website URL
  - [ ] Add Gumroad purchase link
  - [ ] Add topics/tags: productivity, privacy, pdf, desktop, mac
- [ ] Schedule launch for Nov 28, 12:01 AM PST
- [ ] Line up 20+ supporters for upvotes
- [ ] Prepare first comment with additional details

**Post-Launch Actions:**
- [ ] Respond to every comment within 1 hour
- [ ] Share milestones (50, 100, 200 upvotes)
- [ ] Thank supporters publicly
- [ ] Monitor ranking throughout the day

**Expected Results:**
- Top 10: 5,000-10,000 visits
- Top 5: 10,000-20,000 visits
- 2-5% conversion rate

---

### 2. AlternativeTo
**URL:** https://alternativeto.net/  
**Status:** � Submittred (Nov 26, 2025)  
**Impact:** 🔥🔥🔥 CRITICAL  
**Audience:** 10M+ monthly visitors  
**Cost:** Free  
**Review Time:** 24-48 hours

**Why This Matters:**
- High SEO value (ranks well in Google)
- Users actively searching for alternatives
- Permanent listing
- Comparison traffic

**Submission Checklist:**
- [x] Go to https://alternativeto.net/software/add/
- [x] Fill out submission form:
  - [x] Software name: SignKit
  - [x] Category: Office & Productivity > Document Management
  - [x] Description: (300+ words, detailed - see DIRECTORY_SUBMISSION_CONTENT.md)
  - [x] Website: https://signkit.work
  - [x] Platforms: macOS, Windows, Linux
  - [x] License: Commercial ($29, one-time)
  - [x] Tags: pdf-signing, signature-extraction, document-signing, privacy-first, offline-app
- [x] Upload 3 screenshots
- [x] Add as alternative to:
  - [x] Adobe Acrobat DC
  - [x] DocuSign
  - [x] HelloSign
  - [x] PDF Expert
  - [x] Preview (macOS)
  - [x] SignNow
  - [x] PandaDoc
- [x] Submit for review

**Submission Date:** November 26, 2025  
**Expected Approval:** November 27-28, 2025

**Post-Approval Actions:**
- [ ] Encourage users to "like" the listing
- [ ] Respond to comments
- [ ] Update regularly with new features
- [ ] Monitor traffic via UTM parameters

**Expected Results:**
- 500-2,000 visits/month
- High-intent traffic (searching for alternatives)
- Good SEO backlink

---

### 3. Slant
**URL:** https://www.slant.co/  
**Status:** 🔴 Not Started  
**Impact:** 🔥🔥 HIGH  
**Audience:** 5M+ monthly visitors  
**Cost:** Free  
**Review Time:** Instant (community-driven)

**Why This Matters:**
- Community-driven recommendations
- Appears in Google search results
- Users actively comparing options
- Detailed pros/cons format

**Submission Checklist:**
- [ ] Create Slant account
- [ ] Search for relevant questions:
  - "What are the best PDF signature tools?"
  - "What are the best privacy-focused document tools?"
  - "What are the best alternatives to Adobe Acrobat?"
  - "What are the best document signing tools for Mac?"
  - "What are the best offline document tools?"
- [ ] Add SignKit to each relevant question
- [ ] Write detailed pros:
  - [ ] "Complete privacy - all processing is local"
  - [ ] "One-time payment, no subscription"
  - [ ] "Clean, transparent PNG export"
  - [ ] "Built-in PDF signing"
  - [ ] "Cross-platform (Mac, Windows, Linux)"
  - [ ] "Precision selection tools"
- [ ] Write honest cons:
  - [ ] "Not free (but has free trial)"
  - [ ] "Focused on signatures only (not full PDF editor)"
  - [ ] "Requires desktop installation"
- [ ] Add screenshots
- [ ] Link to website

**Post-Submission Actions:**
- [ ] Upvote your own recommendations
- [ ] Encourage users to upvote
- [ ] Respond to questions
- [ ] Update with new features

**Expected Results:**
- 200-1,000 visits/month
- High-quality comparison traffic

---

### 4. Capterra
**URL:** https://www.capterra.com/  
**Status:** 🔴 Not Started  
**Impact:** 🔥🔥 HIGH  
**Audience:** 5M+ monthly visitors (B2B focus)  
**Cost:** Free basic listing  
**Review Time:** 1-2 weeks

**Why This Matters:**
- B2B audience (businesses buying software)
- High-intent traffic
- Review-driven (social proof)
- Good for SEO

**Submission Checklist:**
- [ ] Go to https://www.capterra.com/vendors/sign-up
- [ ] Create vendor account
- [ ] Complete company profile
- [ ] Submit product listing:
  - [ ] Product name: SignKit
  - [ ] Category: Document Management Software
  - [ ] Subcategory: Electronic Signature Software
  - [ ] Description: (500+ words, B2B focused)
  - [ ] Features list (detailed)
  - [ ] Pricing: $29 one-time
  - [ ] Deployment: Desktop (Mac, Windows, Linux)
  - [ ] Company size: All
  - [ ] Industries: All (focus on legal, real estate, healthcare)
- [ ] Upload 10+ screenshots
- [ ] Upload demo video
- [ ] Add integrations (if any)
- [ ] Submit for review

**Post-Approval Actions:**
- [ ] Invite beta users to leave reviews
- [ ] Respond to all reviews
- [ ] Update listing regularly
- [ ] Monitor competitor listings

**Expected Results:**
- 100-500 visits/month
- Higher conversion rate (B2B buyers)
- 5-20 reviews in first 3 months

---

### 5. G2
**URL:** https://www.g2.com/  
**Status:** 🔴 Not Started  
**Impact:** 🔥🔥 HIGH  
**Audience:** 5M+ monthly visitors (B2B focus)  
**Cost:** Free basic listing  
**Review Time:** 1-2 weeks

**Why This Matters:**
- #1 B2B software review platform
- Review-driven (social proof critical)
- High-intent B2B traffic
- Excellent SEO

**Submission Checklist:**
- [ ] Go to https://www.g2.com/products/new
- [ ] Create vendor profile
- [ ] Complete company information
- [ ] Submit product listing:
  - [ ] Product name: SignKit
  - [ ] Category: Electronic Signature
  - [ ] Description: (500+ words, B2B focused)
  - [ ] Key features (detailed list)
  - [ ] Pricing: $29 per user, one-time
  - [ ] Deployment: Desktop
  - [ ] Company size: Small Business, Mid-Market, Enterprise
  - [ ] Industries: Legal, Real Estate, Healthcare, Finance
- [ ] Upload 10+ screenshots
- [ ] Upload demo video
- [ ] Add product documentation links
- [ ] Submit for review

**Post-Approval Actions:**
- [ ] Launch review campaign (need 10+ reviews to appear)
- [ ] Invite customers to leave reviews
- [ ] Respond to all reviews within 24 hours
- [ ] Monitor G2 profile weekly
- [ ] Update with new features

**Expected Results:**
- 50-300 visits/month initially
- Grows significantly with reviews
- Higher conversion rate (B2B)
- Need 10+ reviews to be competitive

---

## 🎯 TIER 2: HIGH PRIORITY (Submit This Week)

### 6. MacUpdate
**URL:** https://www.macupdate.com/  
**Status:** 🔴 Not Started  
**Impact:** 🔥 MEDIUM  
**Audience:** Mac users specifically  
**Cost:** Free  
**Review Time:** 2-3 days

**Submission Checklist:**
- [ ] Go to https://www.macupdate.com/developer/submit
- [ ] Create developer account
- [ ] Submit macOS version:
  - [ ] App name: SignKit
  - [ ] Category: Business > Document Management
  - [ ] Description: (Mac-focused)
  - [ ] Upload .dmg file
  - [ ] System requirements: macOS 11.0+
  - [ ] Pricing: $29 (mention BF deal)
- [ ] Upload screenshots (Mac-specific)
- [ ] Add release notes
- [ ] Submit for review

**Expected Results:**
- 100-500 visits/month
- Mac-specific audience

---

### 7. Softpedia
**URL:** https://www.softpedia.com/  
**Status:** 🔴 Not Started  
**Impact:** 🔥 MEDIUM  
**Audience:** 20M+ monthly visitors  
**Cost:** Free  
**Review Time:** 3-5 days

**Submission Checklist:**
- [ ] Go to https://www.softpedia.com/get/Submit-Software.shtml
- [ ] Submit for each platform:
  - [ ] macOS version
  - [ ] Windows version
  - [ ] Linux version
- [ ] For each submission:
  - [ ] Software name: SignKit
  - [ ] Category: Office > Document Management
  - [ ] Description: (detailed)
  - [ ] Upload installer
  - [ ] Add screenshots
  - [ ] System requirements
  - [ ] License: Shareware ($29)

**Expected Results:**
- 200-1,000 visits/month
- Good for SEO

---

### 8. MacWorld App Directory
**URL:** https://www.macworld.com/  
**Status:** 🔴 Not Started  
**Impact:** 🔥 MEDIUM  
**Audience:** Mac enthusiasts  
**Cost:** Free (editorial)  
**Review Time:** Varies (editorial decision)

**Submission Checklist:**
- [ ] Contact MacWorld editorial team
- [ ] Email: editors@macworld.com
- [ ] Subject: "New Mac App: SignKit - Signature Extraction Tool"
- [ ] Pitch email:
  ```
  Hi MacWorld Team,
  
  I'm launching SignKit, a native Mac app for extracting signatures
  from documents with complete privacy. All processing happens locally
  on the Mac - no cloud uploads.
  
  Key features:
  - Precision signature extraction
  - Clean PNG export with transparency
  - Built-in PDF signing
  - 100% offline processing
  - Native Mac design
  
  Launching for Black Friday at $19 (regular $29).
  
  Would you be interested in featuring it in your app directory
  or reviewing it for MacWorld?
  
  Happy to provide a review copy and press materials.
  
  Best,
  [Your name]
  
  Website: https://signkit.work
  Demo: [YouTube link]
  ```
- [ ] Follow up after 3-5 days if no response

**Expected Results:**
- If featured: 1,000-5,000 visits
- Excellent credibility boost

---

### 9. SourceForge
**URL:** https://sourceforge.net/  
**Status:** 🔴 Not Started  
**Impact:** 🔥 LOW-MEDIUM  
**Audience:** Developers, open-source community  
**Cost:** Free  
**Review Time:** Instant

**Submission Checklist:**
- [ ] Create SourceForge account
- [ ] Create new project: SignKit
- [ ] Set up project page:
  - [ ] Description
  - [ ] Screenshots
  - [ ] Documentation
- [ ] Upload releases:
  - [ ] macOS .dmg
  - [ ] Windows .exe
  - [ ] Linux .tar.gz
- [ ] Add download tracking
- [ ] Set up project website

**Expected Results:**
- 50-200 downloads/month
- Good for open-source credibility

---

### 10. FileHorse
**URL:** https://www.filehorse.com/  
**Status:** 🔴 Not Started  
**Impact:** 🔥 LOW  
**Audience:** General software users  
**Cost:** Free  
**Review Time:** 2-3 days

**Submission Checklist:**
- [ ] Go to https://www.filehorse.com/submit-software/
- [ ] Submit for Mac and Windows
- [ ] Add description and screenshots
- [ ] Upload installers

**Expected Results:**
- 50-200 visits/month

---

## 🎯 TIER 3: NICHE DIRECTORIES (Submit After Launch)

### 11. Privacy Tools Directory
**URL:** https://www.privacytools.io/  
**Status:** 🔴 Not Started  
**Impact:** 🔥 MEDIUM (Niche)  
**Audience:** Privacy-conscious users  
**Cost:** Free  
**Review Time:** Community review

**Why This Matters:**
- Perfect audience for privacy-first messaging
- High-quality, engaged users
- Good for brand positioning

**Submission Checklist:**
- [ ] Review submission guidelines
- [ ] Prepare privacy-focused description
- [ ] Emphasize:
  - [ ] No cloud uploads
  - [ ] No tracking
  - [ ] No data collection
  - [ ] Local processing only
  - [ ] No internet connection required
- [ ] Submit via GitHub or contact form
- [ ] Engage with community

**Expected Results:**
- 50-200 visits/month
- High-quality, privacy-conscious users
- Good brand alignment

---

### 12. Awesome Privacy (GitHub)
**URL:** https://github.com/pluja/awesome-privacy  
**Status:** 🔴 Not Started  
**Impact:** 🔥 LOW-MEDIUM  
**Audience:** Privacy advocates, developers  
**Cost:** Free  
**Review Time:** PR review (1-7 days)

**Submission Checklist:**
- [ ] Fork repository
- [ ] Find appropriate section (Document Tools)
- [ ] Add SignKit entry:
  ```markdown
  - [SignKit](https://signkit.work) - Extract signatures from documents
    with complete privacy. All processing happens locally on your computer.
    No cloud uploads, no tracking. (macOS, Windows, Linux)
  ```
- [ ] Submit pull request
- [ ] Engage with maintainers
- [ ] Respond to feedback

**Expected Results:**
- 20-100 visits/month
- Developer audience
- Good GitHub presence

---

### 13. Legal Tech Directories

#### LawSites
**URL:** https://www.lawsitesblog.com/  
**Status:** 🔴 Not Started  
**Impact:** 🔥 MEDIUM (B2B Legal)  
**Audience:** Legal professionals  
**Cost:** Free (editorial)

**Submission Checklist:**
- [ ] Contact Bob Ambrogi (editor)
- [ ] Email: bob@lawsitesblog.com
- [ ] Pitch legal use cases:
  - Contract signature extraction
  - Legal document management
  - Client signature collection
  - Privacy compliance
- [ ] Offer review copy
- [ ] Provide case studies (if available)

#### Legal Technology Resource Center
**URL:** https://www.americanbar.org/groups/departments_offices/legal_technology_resources/  
**Status:** 🔴 Not Started  
**Impact:** 🔥 MEDIUM (B2B Legal)  
**Audience:** ABA members, legal professionals

**Submission Checklist:**
- [ ] Research submission process
- [ ] Prepare legal-focused materials
- [ ] Submit to directory
- [ ] Follow up

**Expected Results (Combined):**
- 50-200 visits/month
- High-value B2B legal traffic
- Excellent credibility

---

### 14. Real Estate Software Directories

#### BiggerPockets Tools
**URL:** https://www.biggerpockets.com/  
**Status:** 🔴 Not Started  
**Impact:** 🔥 MEDIUM (B2B Real Estate)  
**Audience:** Real estate investors, agents

**Submission Checklist:**
- [ ] Create BiggerPockets account
- [ ] Engage in community
- [ ] Post about SignKit in appropriate forums
- [ ] Offer special pricing for BP members
- [ ] Submit to tools directory (if available)

#### Real Estate Technology Directory
**Status:** 🔴 Not Started  
**Impact:** 🔥 LOW-MEDIUM

**Submission Checklist:**
- [ ] Research real estate tech directories
- [ ] Submit to relevant listings
- [ ] Emphasize use cases:
  - Lease signature extraction
  - Contract management
  - Client document processing

**Expected Results (Combined):**
- 50-200 visits/month
- B2B real estate traffic

---

## 🎯 TIER 4: PAID/PREMIUM (Consider After Revenue)

### 15. AppSumo
**URL:** https://appsumo.com/  
**Status:** 🔴 Not Applied  
**Impact:** 🔥🔥🔥 VERY HIGH  
**Audience:** 1M+ subscribers  
**Cost:** 30% revenue share  
**Review Time:** Application review (selective)

**Why This Matters:**
- Massive audience (1M+ email subscribers)
- Can drive 100-500 sales in 1-2 weeks
- Builds email list
- Social proof and reviews
- Excellent for traction

**When to Apply:**
- After 50+ sales
- After gathering 10+ reviews
- After proving product-market fit
- December 2025 or later

**Application Checklist:**
- [ ] Go to https://appsumo.com/partners/
- [ ] Complete partner application
- [ ] Prepare lifetime deal offer:
  - Tier 1: $39 (vs $29 regular) - Basic features
  - Tier 2: $79 - Plus features (if applicable)
  - Tier 3: $149 - Pro features (if applicable)
- [ ] Prepare product materials:
  - [ ] Detailed description
  - [ ] Screenshots and demo video
  - [ ] Feature comparison
  - [ ] Roadmap
- [ ] Wait for approval (can take weeks)
- [ ] If approved, coordinate launch

**Expected Results:**
- 100-500 sales in 1-2 weeks
- $3,900-$19,500 revenue (at 70% share)
- 500-2,000 email subscribers
- 50-200 reviews
- Massive traction boost

**Considerations:**
- 30% fee is high but worth it for exposure
- Lifetime deal means no recurring revenue
- Can cannibalize regular sales
- Best as one-time traction boost

---

### 16. StackSocial
**URL:** https://stacksocial.com/  
**Status:** 🔴 Not Applied  
**Impact:** 🔥🔥 HIGH  
**Audience:** 500K+ subscribers  
**Cost:** ~30% revenue share  
**Review Time:** Application review

**Similar to AppSumo:**
- Deals marketplace
- Lifetime deal model
- Good for traction
- Apply after initial success

**Application Checklist:**
- [ ] Go to https://stacksocial.com/sell
- [ ] Complete vendor application
- [ ] Prepare deal offer
- [ ] Wait for approval

**Expected Results:**
- 50-200 sales
- $1,950-$7,800 revenue
- Good secondary channel to AppSumo

---

### 17. Mac App Store
**URL:** https://developer.apple.com/  
**Status:** 🔴 Not Started  
**Impact:** 🔥🔥 MEDIUM-HIGH  
**Audience:** All Mac users  
**Cost:** $99/year + 30% revenue share  
**Review Time:** 1-2 weeks per submission

**Why Consider:**
- Massive built-in audience
- Trust and credibility
- Automatic updates
- Sandboxed security
- Discovery through App Store

**Why NOT Recommended (Yet):**
- High fees (30%)
- Strict review process
- Sandboxing may limit functionality
- Can't do lifetime licenses easily
- Complex compliance requirements

**When to Consider:**
- After 6-12 months
- After $10K+ revenue
- After proving desktop model works
- If willing to adapt to sandbox

**If You Decide to Submit:**
- [ ] Enroll in Apple Developer Program ($99/year)
- [ ] Review App Store guidelines
- [ ] Adapt app for sandboxing
- [ ] Prepare for review process
- [ ] Submit via App Store Connect

---

### 18. Setapp
**URL:** https://setapp.com/  
**Status:** 🔴 Not Applied  
**Impact:** 🔥🔥 MEDIUM-HIGH  
**Audience:** 2M+ subscribers  
**Cost:** Revenue share (30-40%)  
**Review Time:** Application review (selective)

**Why Consider:**
- Passive income stream
- Exposure to 2M+ Mac users
- No marketing needed
- Recurring revenue
- Professional platform

**Why NOT Recommended (Yet):**
- Selective (not all apps accepted)
- Lower per-user revenue
- Must maintain subscription model
- Revenue depends on usage metrics

**When to Apply:**
- After 3-6 months
- After gathering excellent reviews
- After proving product quality
- If seeking passive income

**Application Checklist:**
- [ ] Go to https://setapp.com/developers
- [ ] Review requirements
- [ ] Prepare application:
  - [ ] App must be high quality
  - [ ] Must have good reviews
  - [ ] Must fit Setapp audience
- [ ] Submit application
- [ ] Wait for review (can take months)

**Expected Results (If Accepted):**
- $500-$2,000/month passive income
- Exposure to 2M+ users
- Good for long-term stability

---

## 📊 TRACKING & ANALYTICS

### UTM Parameters for Each Directory

Use these UTM parameters to track traffic from each directory:

```
Product Hunt: ?utm_source=producthunt&utm_medium=listing&utm_campaign=bfcm2025
AlternativeTo: ?utm_source=alternativeto&utm_medium=listing&utm_campaign=directories
Slant: ?utm_source=slant&utm_medium=listing&utm_campaign=directories
Capterra: ?utm_source=capterra&utm_medium=listing&utm_campaign=directories
G2: ?utm_source=g2&utm_medium=listing&utm_campaign=directories
MacUpdate: ?utm_source=macupdate&utm_medium=listing&utm_campaign=directories
Softpedia: ?utm_source=softpedia&utm_medium=listing&utm_campaign=directories
```

### Analytics Dashboard

Track these metrics for each directory:

- **Traffic:** Visits from each source
- **Conversion Rate:** % of visitors who purchase
- **Revenue:** Total revenue from each source
- **Cost Per Acquisition:** If paid
- **ROI:** Revenue / Cost

### Monthly Review

Review directory performance monthly:

- [ ] Which directories drive most traffic?
- [ ] Which have highest conversion rates?
- [ ] Which are worth maintaining?
- [ ] Which should we invest more in?
- [ ] Which should we abandon?

---

## 🎯 PRIORITY ACTION PLAN

### This Week (Nov 21-24)

**Friday, Nov 22:**
- [ ] Submit to AlternativeTo
- [ ] Submit to Slant
- [ ] Submit to Capterra
- [ ] Submit to G2
- [ ] Set up Product Hunt listing (don't publish)

**Saturday, Nov 23:**
- [ ] Submit to MacUpdate
- [ ] Submit to Softpedia
- [ ] Submit to SourceForge
- [ ] Submit to FileHorse

**Sunday, Nov 24:**
- [ ] Follow up on pending submissions
- [ ] Prepare Product Hunt launch materials
- [ ] Test all directory links

**Monday, Nov 25:**
- [ ] Final Product Hunt prep
- [ ] Schedule PH launch for Nov 28

### Launch Week (Nov 28 - Dec 4)

**Thursday, Nov 28:**
- [ ] Launch on Product Hunt
- [ ] Monitor all directory traffic
- [ ] Respond to comments/reviews

**Friday-Sunday:**
- [ ] Continue engagement
- [ ] Monitor analytics
- [ ] Adjust messaging based on feedback

**Monday, Dec 2:**
- [ ] Review week's performance
- [ ] Identify top-performing directories
- [ ] Plan next submissions

### Month 2 (December)

- [ ] Apply to AppSumo
- [ ] Submit to niche directories
- [ ] Gather reviews on Capterra/G2
- [ ] Optimize top-performing listings

---

## 📞 SUPPORT & RESOURCES

**Directory Submission Help:**
- Indie Hackers: https://www.indiehackers.com/
- r/SideProject: https://reddit.com/r/SideProject
- Product Hunt Makers: https://www.producthunt.com/makers

**Review Management:**
- Capterra Review Guide: https://www.capterra.com/reviews/
- G2 Review Guide: https://www.g2.com/reviews/

**Analytics:**
- Google Analytics: https://analytics.google.com/
- UTM Builder: https://ga-dev-tools.web.app/campaign-url-builder/

---

**Last Updated:** November 26, 2025  
**Next Review:** Daily until launch, then weekly  
**Owner:** @pranaysuyash

---

## 📈 RECENT ACTIVITY LOG

**November 26, 2025:**
- ✅ Submitted to AlternativeTo (awaiting 24-48hr review)
- 📝 Created comprehensive directory submission content (DIRECTORY_SUBMISSION_CONTENT.md)

**November 21, 2025:**
- 🟢 Submitted to BetaList (awaiting review)

---

🚀 **LET'S GET LISTED EVERYWHERE!**
