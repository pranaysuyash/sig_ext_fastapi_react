...
## Local testing with virtualenv (macOS / Linux)

If you already have a Python virtual environment active, use it to run the server and smoke tests.

1. Activate your venv (if not active):

```bash
# If your venv is in .venv
source .venv/bin/activate
```

2. Start the simple server in the background (use `nohup` if you want it detached):
...
4. (Optional) Run the Playwright E2E tests (Node required):
...
This uses your local venv and will not change the repository. To stop the server manually: `pkill -f serve.py` or `fg` then Ctrl+C.
